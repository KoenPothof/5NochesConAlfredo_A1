...
std::shared_ptr<GameObject> roomObjectA;
	...
	glfwSetKeyCallback(window, [](GLFWwindow* window, int key, int scancode, int action, int mods)
		{
			...
			if (key == GLFW_KEY_L && leftDoorClosed) // Close left door
				roomObjectA->getComponent<RoomComponent>()->closeDoor(RoomComponent::LEFTDOOR);
			leftDoorClosed = true;

			if (key == GLFW_KEY_L && !leftDoorClosed) // Open left door
				roomObjectA->getComponent<RoomComponent>()->openDoor(RoomComponent::LEFTDOOR);
			leftDoorClosed = false;

			if (key == GLFW_KEY_R && rightDoorClosed) // Close right door
				roomObjectA->getComponent<RoomComponent>()->closeDoor(RoomComponent::RIGHTDOOR);
			rightDoorClosed = true;

			if (key == GLFW_KEY_R && !rightDoorClosed) // Open right door
				roomObjectA->getComponent<RoomComponent>()->openDoor(RoomComponent::RIGHTDOOR);
			rightDoorClosed = false;

			if (key == GLFW_KEY_R && !leftDoorClosed) // Checks to see if the opposite door is already closed and closes both doors if thats the case.
				roomObjectA->getComponent<RoomComponent>()->closeDoor(RoomComponent::BOTHDOORS);

			if (key == GLFW_KEY_L && !rightDoorClosed) // Checks to see if the opposite door is already closed and closes both doors if thats the case.
				roomObjectA->getComponent<RoomComponent>()->closeDoor(RoomComponent::BOTHDOORS);
...
void initRoom()
{
	...
	roomObjectA = std::make_shared<GameObject>();
	roomObjectA->position = glm::vec3(0, 0, 0);
	auto roomComponentA = std::make_shared<RoomComponent>(10, 10, 2, RoomComponent::FRONTBACK);
	roomComponentA->openDoor(RoomComponent::LEFTDOOR);
	roomComponentA->openDoor(RoomComponent::RIGHTDOOR);
